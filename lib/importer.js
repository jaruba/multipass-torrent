/* Collect info hashes to queue for indexing
 */
 
var needle = require("needle");
var gunzip = require("gunzip-maybe");
var url = require("url");

var importers = { 
	dump: require("../importers/dump"),
	xmlRss: require("../importers/xml-rss"),
	generic: require("../importers/generic")
};

var db = require("../lib/db");

function collect(source, callback, onHash)
{
	var status = { found: 0, start: Date.now() }, stream, response;

	stream = response = needle.get(source.url, { follow_max: 4 })
	.on("headers", function(headers) {
		// Some sources can be gunzipped twice (one for request, another for being a .txt.gz)
		stream = stream.pipe(gunzip()).pipe(gunzip());

		var filename = headers["content-disposition"] || url.parse(source.url).pathname;
		var detectedType = "generic";
		if (headers["content-type"] && headers["content-type"].match("xml")) detectedType = "xmlRss";
		if (filename.match(".txt.gz$")) detectedType = "dump";

		// NOTE: if you prefer the full response rather than a stream, use in your importer:
		// var parts = []; stream.on("data", function(d) { parts.push(d) }); stream.on("end", function() { Buffer.concat(parts).toString() })
		// A stream is used because of the potential size of the import source

		var type = status.type = importers[source.type] ? source.type : detectedType;

		// Pass on to the importer
		stream = importers[type](stream, source);

		// Collection results
		stream.on("infoHash", function(hash, extra) {
			status.found++;
			if (onHash) onHash(hash.toLowerCase(), extra);
		});

		stream.on("end", function() { response.end(); stream.removeAllListeners(); status.end = Date.now(); callback(null, status) });
	})
	.on("error", function(err) { callback(err) });
};

module.exports = { collect: collect };
