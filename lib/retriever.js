...
var downloader = async.queue(function(task, cb)
{
    ...
    async.whilst(function() { return !result && sources.length }, function(innerCb)
    {
        ...
        needle(source.formatted ? source.formatted : util.format(source.url, [ task.infoHash.toUpperCase() ]), {
            timeout: 4000,
        ...
        .on("readable", function(res)
        {
            ...
            if (res.statusCode != 200) return innerCb(); /* Go on to the next source */
            res = res.pipe(gunzip());
