...
function updateIndexes(entry, callback)
{
	...
	if (torrent.files) torrent.files.forEach(function(x) { 
		var hash = getHash(x);
		//( (maxSeeders > MIN_SEED_TO_INDEX) ? db.indexes.meta.insert : db.indexes.meta.delete)(hash, torrent.infoHash);
		db.indexes.meta.insert(hash, torrent.infoHash);
	});
	callback && callback();
}
