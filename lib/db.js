...
db.listenReplications = function(id) {
	...
	ssdpServer.addUSN("urn:schemas-upnp-org:service:MultiPassTorrent:1");
...
db.findReplications = function(id) {
	...
	ssdp.search("urn:schemas-upnp-org:service:MultiPassTorrent:1");
};
...
dht.on("peer", onpeer);
ssdp.on("response", function(meta, status, matchine) { 
	console.log(meta);/* onpeer(url.parse(meta.LOCATION).host) */ 
});
