...
/*
db.createValueStream().on("data", function(vals) {
	var latest = vals.sort(function(a,b) { return b.updated.getTime() - a.updated.getTime() })[0];
	updateIndexes({ value: latest });
});
*/
...
db.log.createReadStream({ valueEncoding: "binary" }).on("data", function(entry) {
	entry = messages.Entry.decode(entry.entry);
	updateIndexes({ value: mmmEnc.decode(entry.value) });
});
