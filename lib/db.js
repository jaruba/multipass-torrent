...
/* Querying
 */
db.lookup = function(query, count, callback)
{
	callback(null, db.indexes.meta.search(getHashes(query)[0])
...
{
	db.lookup(query, items, function(err, matches) {
		if (err) return callback(err);
		async.map(matches, function(x, cb) { 
