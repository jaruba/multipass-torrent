...
function index(task, options, callback)
{
	...
	if (!options.force && task.source && torrent.sources[task.source.url]) return callback(null, torrent, true);
	...
	(options.retrieve || retriever.retrieve)(task.infoHash, { 
		important: db.getMaxPopularity(torrent) > cfg.minSeedImportant, // WARNING: what if seedleech is not called yet?
		// url: // TODO for yifi
	}, function(err, tor) {
