...
function index(task, options, callback)
{
	...
	(options.retrieve || require("../lib/retriever").retrieve)(task.infoHash, { 
		// WARNING: what if seedleech is not called yet?
		important: (task.source && task.source.important) || task.important,
		...
		torrent.files = torrent.files
			.map(function(file, idx) { 
				if (! file.path) throw new Error('sanity check failed - file does not have path: '+JSON.stringify(torrent));
				return _.extend(file, { idx: idx, name: path.basename(file.path) });
			})
		...
		(function(next) {
			...
			async.map(torrent.files, function(file, cb) {
				...
				nameToImdb(_.extend({ hintUrl: task.hints && task.hints.url }, file), function(err, imdb_id) {
